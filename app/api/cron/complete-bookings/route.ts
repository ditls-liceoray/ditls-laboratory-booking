...
export async function POST(request: Request) {
  ...
  if (!secret || !/^[a-fA-F0-9]{64}$/.test(secret)) {
    return Response.json(
      { success: false, error: 'Completion scheduler is not configured.' },
      { status: 503, headers },
    );
  }
  ...
  if (!url || !serviceRoleKey) {
    return Response.json(
      { success: false, error: 'Completion database access is not configured.' },
      { status: 503, headers },
    );
  }
  ...
  try {
    ...
    if (error || !Number.isInteger(data) || data < 0) {
      return Response.json(
        { success: false, error: 'Automatic completion failed; retry is safe.' },
        { status: 500, headers },
      );
    }

    return Response.json({ success: true, completed: data }, { headers });
  } catch {
    return Response.json(
      { success: false, error: 'Automatic completion failed; retry is safe.' },
      { status: 500, headers },
