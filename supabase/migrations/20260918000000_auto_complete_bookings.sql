...
REVOKE ALL
ON SCHEMA booking_automation_private
...
REVOKE ALL
ON TABLE booking_automation_private.completion_context
FROM PUBLIC, anon, authenticated, service_role;

ALTER TABLE booking_automation_private.completion_context
...
BEGIN
    ...
    SELECT c.booking_id
    INTO v_completion_booking_id
    FROM booking_automation_private.completion_context c
    WHERE c.transaction_id = txid_current();

    IF FOUND THEN

        IF v_completion_booking_id = OLD.id THEN

            IF OLD.status = 'approved'
               AND NEW.status = 'completed'
               AND (
                    (OLD.booking_date + OLD.end_time)
            ...
            THEN
                ...
                -- Consume the authorization context.
                DELETE FROM booking_automation_private.completion_context
    ...
    IF v_is_admin THEN
        ...
        IF v_old_status = 'pending'
           AND v_new_status IN ('approved', 'rejected', 'cancelled')
        THEN
            RETURN NEW;
        END IF;
        ...
        IF v_old_status = 'approved'
           AND v_new_status IN ('completed', 'cancelled')
        THEN
            RETURN NEW;
        END IF;
        ...
        RAISE EXCEPTION
            'Invalid status transition: cannot change booking from % to %',
            v_old_status,
            v_new_status;
    ...
    IF NEW.teacher_id = v_current_teacher_id THEN
        ...
        -- Teachers may only cancel their own pending bookings.
        IF v_old_status = 'pending'
...
BEGIN
    ...
    FOR v_booking IN
        ...
        INNER JOIN public.laboratories l
            ON l.id = b.laboratory_id
    ...
    LOOP
        ...
        INSERT INTO booking_automation_private.completion_context (
            transaction_id,
            booking_id
        )
        VALUES (
        ...
        IF NOT FOUND THEN
            ...
            RAISE EXCEPTION
                'Automatic booking completion did not update the locked booking';
        ...
        IF EXISTS (
            ...
            RAISE EXCEPTION
                'Automatic booking completion authorization was not consumed';
        ...
        VALUES (
            ...
            format(
                ...
                'The booking was automatically marked as completed ' ||
...
REVOKE ALL
ON FUNCTION public.complete_expired_bookings()
